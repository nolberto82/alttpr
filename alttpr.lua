...
function map_display_item_counts()
...
function map_zoom_toggle()
	setregister(regs[curremu]["a"], 0x00)
end

function map_frame_counter() --disable overworld map frame counter
	if u8(const.SUB_TASK) == 7 and u8(const.NMI_FLAG) > 0 and u8(const.ZOOM_MODE) == 0 then
		w8(0x1a, 0xff)
...
function openoverworldmapindoors()
...
function close_overworld_map_indoors()
...
function disable_map()
	...
	if screen == 0x80 or screen == 0x81 then
		setregister(regs[curremu]["a"], 0x00)
	end
end

function open_overworld_map_outside() --
	world = getworld()
end
...
if curremu == const.GMULATOR or curremu == const.MESEN then
	addmemcallback(map_display_item_counts, callbackexec, 0x8abf86) --main
	addmemcallback(map_zoom_out, callbackexec, 0x8abcfa)
	addmemcallback(map_zoom_toggle, callbackexec, 0x8abb34)
	addmemcallback(map_frame_counter, callbackexec, 0x808053)
	addmemcallback(sprites_8x8, callbackexec, 0x80805d)
	addmemcallback(openoverworldmapindoors, callbackexec, 0x828801)
	addmemcallback(disable_map, callbackexec, 0xa483cd)
	addmemcallback(open_overworld_map_outside, callbackexec, 0x82a465)
	addmemcallback(close_overworld_map_indoors, callbackexec, 0x8abc8f)
	addmemcallback(close_dungeon_map, callbackexec, 0x8aefd8)
	addmemcallback(set_world, callbackexec, 0x82e9f2)
	addmemcallback(change_world, callbackexec, 0x8aba6c)
	addmemcallback(change_world_2, callbackexec, 0x8aba9b)
	addmemcallback(links_position, callbackexec, 0x8ac3b8)
	addmemcallback(links_position_2, callbackexec, 0x80f806)
	addmemcallback(draw_link_low_priority, callbackexec, 0x8abf9d)
	addmemcallback(set_travel_location, callbackexec, 0x82ea2f)
	addeventcallback(update, callbackframe) --update	
